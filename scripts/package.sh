#!/bin/sh
# Build a release: dist/msl-<version>-macos-arm64.tar.gz, dist/msl-<version>.pkg,
# dist/update.json (for `msl --update`) and packaging/homebrew/msl.rb.
#
...
cat > packaging/homebrew/msl.rb <<RUBY
...
class Msl < Formula
  ...
  url "$BASE_URL/$NAME-macos-arm64.tar.gz"
  sha256 "$SHA"
  version "$VERSION"
...
echo "tarball  $TARBALL  sha256 $SHA"
echo "pkg      $PKG"
echo "update   dist/update.json"
echo "brew     packaging/homebrew/msl.rb"
