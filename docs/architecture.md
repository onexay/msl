...
| `dnsTunneling` | Guest stub at `10.255.255.254:53` (UDP+TCP) relays queries over vsock; `msld` answers with `DNSServiceQueryRecord` (mDNSResponder), so VPN split DNS, `/etc/resolver`, `.local` and the Mac's hosts file work. Loopback/link-local answers are dropped. `[msl2] dnsTunneling=false` falls back to vmnet DNS. |
