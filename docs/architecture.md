...
| WSL | MSL |
|---|---|
| Distro storage | One shared ext4 data disk (a sparse raw file; 256 GiB or `defaultVhdSize`, never more than the macOS volume). Each distro is a directory on it. `--manage --resize` grows the shared disk; `--move` isn't supported, since there's no per-distro disk to move. |
...
| Localhost forwarding (NAT) | The guest streams its listening TCP ports (`MiniInit.WatchPorts`); `msld` binds `127.0.0.1`/`::1` for each (skipping ports in use on macOS) and relays connections over vsock to a guest forwarder. `host.internal` in the generated `/etc/hosts` points at the vmnet gateway. |
