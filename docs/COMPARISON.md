...
- **ArcBox:** an open-source Rust Docker/OrbStack alternative with Linux machines, a Finder-visible `~/ArcBox`, and switchable VZ or custom Hypervisor.framework backends. Its [memory-ratchet write-up](https://arcbox.dev/blog/macos-vm-memory-ratchet) matches msl's own measurements ([memory-reclaim.md](memory-reclaim.md)): no Virtualization.framework runtime gets freed RAM back to macOS. [Repo](https://github.com/arcboxlabs/arcbox).
