...
- **Docs:** update `README.md` and `docs/` in the same pull request as the behaviour change, and add a line to `CHANGELOG.md` under *Unreleased*. Documentation files at the root are UPPERCASE (`SECURITY.md`); files under `docs/` are lowercase snake_case (`getting_started.md`). `scripts/check-links.py` checks relative links and headings, and `swift test` checks that `docs/cli.md` matches `msl --help`; CI runs both. The same files are the website at [onexay.github.io/msl](https://onexay.github.io/msl/), built by MkDocs (`mkdocs.yml`; `scripts/docs_hooks.py` points links to files outside `docs/` at the right page or at GitHub). Preview it with `pip install -r docs/requirements.txt && mkdocs serve`; the Docs workflow builds it with `--strict` on every PR and deploys it from `main`.
